...
void main() async {
  ...
  final languageFilesDir = Directory('assets/lang');
  final List<FileSystemEntity> languageFilesEntities =
      await languageFilesDir.list().toList();
  final fullLanguageStringsEntity = languageFilesEntities.firstWhere(
    (e) => e.path.endsWith("full-lang.txt"),
  );
  final newLanguageStringsEntity = languageFilesEntities.firstWhere(
    (e) => e.path.endsWith("new-lang.txt"),
  );

  //
  print("Loadding New Lang Strings");
  File newLanguageStringsFile = File(newLanguageStringsEntity.path);
  List<String> newLangStrings = (await newLanguageStringsFile.readAsString())
      .split("\n");
  //
